...
\[
\operatorname{weight}(x) = \frac{\sum_{i=1}^{N} \frac{\operatorname{weight}_i}{d(x, x_i)^p}}{\sum_{i=1}^{N} \frac{1}{d(x, x_i)^p}}
\]

где \(d(x, x_i)^p\) — расстояние между точками, а при \(d(x, x_i) = 0\) значение \(\operatorname{weight}(x)\) принимается равным \(\operatorname{weight}_i\).
